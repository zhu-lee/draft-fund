...
public class CodeGenerator {
    ...
    public String getCode() {
        ...
        code.append("public class " + className + " implements mtime.lark.pb.Codec");
    ...
    private void genImportCode(StringBuilder code) {
        ...
        code.append("import mtime.lark.pb.utils.*;\n");
        code.append("import java.lang.reflect.*;\n");
        code.append("import mtime.lark.pb.*;\n");
    ...
    /**
     * generate <code>read</code> method source code
     *
     * @return
     */
    private String getDecodeMethodCode() {
        ...
        code.append(" read(byte[] bb) throws IOException {\n");
        ...
        for (FieldInfo field : fields) {
            ...
            if (field.getType() != FieldType.DEFAULT) {
                code.append("if (tag == ").append(
                        CodedConstant.makeTag(field.getOrder(), field.getType().getInternalFieldType()
                                .getWireType()));
                code.append(") {\n");
            } else {
                code.append("if (tag == CodedConstant.makeTag(").append(field.getOrder());
                code.append(",WireFormat.").append(field.getType().getWireFormat()).append(")) {\n");
            }
            String t = field.getType().getType();
            ...
            if (field.getType() == FieldType.ENUM) {
                ...
                express = "new Date(input.read" + t + "())";
            } else if (field.getType() == FieldType.LTIME) {
                express = "LocalDateTime.ofInstant(Instant.ofEpochMilli(input.read" + t + "()), ZoneId.systemDefault())";
            } else {
                express = "input.read" + t + "()";
            }
            if (isList && field.getType() == FieldType.OBJECT) {
                Type type = field.getField().getGenericType();
                if (type instanceof ParameterizedType) {
                    ParameterizedType ptype = (ParameterizedType) type;
            ...
            } else if (field.getType() == FieldType.OBJECT) {
            ...
            if (field.getType() == FieldType.BYTES) {
                express += ".toByteArray()";
            }
...
//            if ("int".equals(cls.getSimpleName()) || "Integer".equals(cls.getSimpleName())) {
//                return;
//            }
    ...
    private String getEncodeMethodCode() {
        ...
        for (FieldInfo field : fields) {
            ...
            // check type
            if (!isList) {
                checkType(field.getType(), field.getField());
            }
            ...
            // define field
            code.append(CodedConstant.getMappedTypeDefined(field.getOrder(), field.getType(),
            ...
            code.append("if (!CodedConstant.isNull(").append(getAccessByField("t", field.getField(), cls))
                    .append("))\n");
            code.append("{\nsize+=");
            code.append(CodedConstant.getMappedTypeSize(field, field.getOrder(), field.getType(), isList, debug,
                    outputPath));
            code.append("}\n");
            if (field.isRequired()) {
        ...
        for (FieldInfo field : fields) {
            boolean isList = isListType(field.getField());
            // set write to byte
            code.append(CodedConstant.getMappedWriteCode(field, "output", field.getOrder(), field.getType(),
                    isList));
        }
    ...
    protected String getAccessByField(String target, Field field, Class<?> cls) {
        ...
        try {
            ...
            LOGGER.error("find getter failed", e);
        }
    ...
    protected String getSetToField(String target, Field field, Class<?> cls, String express, boolean isList) {
        ...
        try {
            ...
            LOGGER.error("find setter failed", e);
        }
