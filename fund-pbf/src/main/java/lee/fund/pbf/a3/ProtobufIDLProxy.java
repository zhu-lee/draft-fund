...
import lee.fund.pbf.a3.utils.JDKCompilerHelper;
import lee.fund.pbf.a3.utils.StringUtils;
...
public class ProtobufIDLProxy {
     ...
     */
    private static final Map<String, String> fieldTypeMapping = new HashMap<>();
    ...
    private static List<Class> createClass(ProtoFile protoFile, boolean multi, boolean debug, boolean generateSouceOnly,
        ...
        if (generateSouceOnly) {
            if (sourceOutputDir == null) {
                throw new UncheckedException("param 'sourceOutputDir' is null.");
            }

            if (!sourceOutputDir.isDirectory()) {
                throw new UncheckedException("param 'sourceOutputDir' should be a exist file directory.");
            }
        }

        List<TypeElement> types = protoFile.typeElements();
        if (types == null || types.isEmpty()) {
            throw new UncheckedException("No message defined in '.proto' IDL");
        }
        ...
        if (!multi && count != 1) {
            throw new UncheckedException("Only one message defined allowed in '.proto' IDL");
        }
    ...
    private static void writeSourceCode(CodeDependent cd, File sourceOutputDir) {
        ...
        try (FileOutputStream fos = new FileOutputStream(new File(f, cd.name + ".java"))) {
    ...
    private static CodeDependent createCodeByType(ProtoFile protoFile, EnumElement type, boolean topLevelClass) {
        ...
        if (topLevelClass) {
            ...
            code.append("\n");
            // add import;
            code.append("import mtime.lark.pb.EnumReadable;\n");
        }
    ...
    private static CodeDependent createCodeByType(ProtoFile protoFile, MessageElement type, Set<String> enumNames,
        ...
        if (topLevelClass) {
            ...
            code.append("\n");
            // add import;
            code.append("import mtime.lark.pb.FieldType;\n");
            code.append("import mtime.lark.pb.EnumReadable;\n");
            code.append("import mtime.lark.pb.annotation.Protobuf;\n");
        }
        ...
        for (FieldElement field : fields) {
            ...
            FieldType fType = typeMapping.get(field.type());
            String javaType;
            if (fType == null) {
                javaType = field.type() + DEFAULT_SUFFIX_CLASSNAME;
                if (!isNestedTypeDependency(field.type().toString(), checkNestedTypes)) {
            ...
            if (defaultOption != null) {
                ...
                if (defaultValue instanceof EnumConstantElement) {
                    EnumConstantElement enumValue = (EnumConstantElement) defaultValue;
                    code.append(javaType).append(".").append(enumValue.name());
    ...
    private static void generateProtobufDefinedForField(StringBuilder code, FieldElement field, Set<String> enumNames) {
        code.append("@").append(ProtoField.class.getSimpleName()).append("(");

        String type = fieldTypeMapping.get(field.type().toString());
        if (type == null) {
            if (enumNames.contains(field.type().toString())) {
                type = "FieldType.ENUM";
            } else {
