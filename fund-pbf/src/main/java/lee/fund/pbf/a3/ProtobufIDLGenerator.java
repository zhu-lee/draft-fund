...
/**
 * 
 * Utility class for generate protobuf IDL content from @{@link ProtoField}
 * 
 * @author xiemalin
 * @since 1.0.1
 */
public class ProtobufIDLGenerator {
    ...
    private static void generateIDL(StringBuilder code, Class<?> cls, Set<Class<?>> cachedTypes,
        ...
        List<FieldInfo> fieldInfos = ProtobufProxyUtils.processDefaultValue(cls, fields);
        for (FieldInfo field : fieldInfos) {
            if (field.hasDescription()) {
                code.append("// ").append(field.getDescription()).append("\n");
            }
            if (field.getType() == FieldType.OBJECT || field.getType() == FieldType.ENUM) {
                if (field.isList()) {
                        ...
                        if (actualTypeArguments != null && actualTypeArguments.length > 0) {
                            ...
                            if (targetType instanceof Class) {
                                ...
                                if (ProtobufProxyUtils.isScalarType(c)) {
                                    ...
                                    FieldType fieldType = ProtobufProxyUtils.getFieldType(c);
                ...
                String type = field.getType().getType().toLowerCase();
    ...
    private static void generateEnumIDL(StringBuilder code, Class<Enum> cls) {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (value instanceof EnumValueSupport) {
                    code.append(((EnumValueSupport) value).value());
                } else {
