...
public class CodedConstant {
    ...
    private static Codec<FileDescriptorProtoPOJO> descriptorCodec = CodecFactory.get(FileDescriptorProtoPOJO.class);
    ...
    public static String getMappedTypeSize(FieldInfo field, int order, FieldType type, boolean isList, boolean debug,
        ...
        if (type == FieldType.ENUM) {
            if (EnumValueSupport.class.isAssignableFrom(field.getField().getType())) {
    ...
    public static int computeSize(int order, Object o, FieldType type, boolean list, boolean debug, File path) {
        ...
        if (type == FieldType.STRING) {
            ...
            if (o instanceof EnumValueSupport) {
                size = CodedOutputStream.computeInt32SizeNoTag(((EnumValueSupport) o).value());
    ...
    public static void writeObject(CodedOutputStream out, int order, FieldType type, Object o, boolean list)
        ...
        if (type == FieldType.BOOL) {
            ...
            int value = 0;
            if (o instanceof EnumValueSupport) {
                value = ((EnumValueSupport) o).value();
            } else if (o instanceof Enum) {
     ...
     * @return true if is null
     */
    public static boolean isNull(Date o) {
        return o == null;
    }

    /**
     * check LocalDateTime is null
     *
     * @param o to check
     * @return true if is null
     */
    public static boolean isNull(LocalDateTime o) {
        return o == null;
    }
    ...
    public static String getEnumName(Enum[] e, int value) {
        if (e != null) {
            int toCompareValue;
            for (Enum en : e) {
                if (en instanceof EnumValueSupport) {
                    toCompareValue = ((EnumValueSupport) en).value();
                } else {
    ...
    public static Descriptor getDescriptor(Class<?> cls) throws IOException {
        ...
        List<com.squareup.protoparser.TypeElement> typeElements = file.typeElements();
        if (typeElements != null) {

            for (com.squareup.protoparser.TypeElement typeElement : typeElements) {
            ...
            for (com.squareup.protoparser.TypeElement typeElement : typeElements) {
                ...
                if (typeElement instanceof MessageElement) {
                    fileDescriptorProto.messageTypes.add(
                            getDescritorProtoPOJO(fileDescriptorProto, (MessageElement) typeElement, messageSet, enumSet));
                } else if (typeElement instanceof EnumElement) {
    ...
    private static DescriptorProtoPOJO getDescritorProtoPOJO(FileDescriptorProtoPOJO fileDescriptorProto,
        ...
        List<com.squareup.protoparser.FieldElement> fields = typeElement.fields();
        if (fields != null) {
            FieldDescriptorProtoPOJO fieldDescriptorProto;
            for (com.squareup.protoparser.FieldElement fieldElement : fields) {
                ...
                OptionElement defaultOption = fieldElement.getDefault();
                if (defaultOption != null) {
                    fieldDescriptorProto.defaultValue = defaultOption.value().toString();
                }

                try {
                    fieldDescriptorProto.type = Type.valueOf("TYPE_" + type.toString().toUpperCase());

                } catch (Exception e) {
                    if (messageSet.contains(type.toString())) {
        ...
        List<com.squareup.protoparser.TypeElement> nestedElements = typeElement.nestedElements();
        if (nestedElements != null) {
            for (com.squareup.protoparser.TypeElement nestedTypeElement : nestedElements) {
    ...
    private static EnumDescriptorProtoPOJO getDescritorProtoPOJO(FileDescriptorProtoPOJO fileDescriptorProto,
        ...
        if (values != null) {
            ...
            for (com.squareup.protoparser.EnumConstantElement fieldElement : values) {
